...
if __name__ == "__main__":
    ...
    subset_of_words = actual_words[:]# actual_words[0::200]
    sim_percent_comp = 0
    best_average = 100
    list_of_tries = []
    best_words = sorted_words[-2:]
    for starting_guess in best_words:
        ...
        if best_average > tries_tally/len(subset_of_words):
            best_average = tries_tally/len(subset_of_words)
            best_word = starting_guess

        

        print("The best starting word so far is: " + best_word + " which took an average of " + str(best_average) + " guesses")    
        ...
        x = range(0, len(list_of_tries))
        plot.scatter(x, list_of_tries)
        plot.xlabel('Sorted Starting Guesses')
        plot.ylabel('Average Guesses to Solve')
        plot.title('Guesses to Solve vs Starting Guess')
        plot.show()
